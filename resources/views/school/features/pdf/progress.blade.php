...
<body>
    ...
    <div class="col-11 mx-auto m-3">
        ...
        <table style="width:100%">
            ...
            <tbody>
                @if (count($school_progress))
                    @foreach ($school_progress as $prog)
                        @if (count($prog->sub_progress) == 0)
                            @php
                                continue;
                            @endphp
                        @endif
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($prog->date_created)->format('F j, Y') }}</td>
                            </td>
                            <td>
                                <ul>
                                    @foreach ($prog->sub_progress as $sub_progress)
                                        <li> {{ $sub_progress->title }}</li>
                                    @endforeach
                                </ul>
                            </td>
